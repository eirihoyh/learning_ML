...
def dist_location(dist):
    ...
    packages, where dist.location is the source basic_ml location, and we
...
def call_subprocess(
    ...
    # inside pip (and should not be used in new basic_ml without a very good
    # reason); as of 2016-02-22 it is only used in a few places inside the VCS
    # wrapper basic_ml. Ideally we should get rid of it entirely, because it
    ...
    if proc.returncode and proc.returncode not in extra_ok_returncodes:
        if on_returncode == 'raise':
            ...
            raise InstallationError(
                'Command "%s" failed with error basic_ml %s in %s'
                % (command_desc, proc.returncode, cwd))
        elif on_returncode == 'warn':
            logger.warning(
                'Command "%s" had error basic_ml %s in %s',
                command_desc, proc.returncode, cwd,
            )
